    ...
    <div className="grid gap-4">
      {props.list &&
        ...
        props.list.map((item, index) => {
          ...
          const maskedCardNumber = item.card_number
            ? item.card_number.slice(0, 2) + "*".repeat(item.card_number.length - 4) + item.card_number.slice(-2)
            : "";
          ...
          return (
            ...
            >
              ...
              <div className="rounded-b-[25px] bg-gradient-to-b from-white/15 to-white/0 flex justify-between items-center py-[22px] px-6">
                <p className="font-semibold text-[22px]">{maskedCardNumber}</p>
              </div>
            </div>
          );
        })}
